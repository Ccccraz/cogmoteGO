from dataclasses import dataclass
import random
from datetime import datetime, timedelta
import asyncio
import httpx

@dataclass
class TrialData:
    count: int
    trial_start_time: str
    trial_end_time: str
    result: bool

async def send_data(client: httpx.AsyncClient, data: TrialData) -> bool:
    try:
        response = await client.post(
            "http://localhost:9012/api/broadcast/data/default",
            json={
                "count": data.count,
                "start_time": data.trial_start_time,
                "end_time": data.trial_end_time,
                "result": data.result
            },
            timeout=10.0
        )
        response.raise_for_status()
        print(f"Data #{data.count} sent successfully!")
        return True
    except httpx.HTTPError as e:
        print(f"Failed to send data #{data.count}: {str(e)}")
        return False

async def gen_data(count: int, last_end_time: datetime | None) -> tuple[TrialData, datetime, int]:
    start_time = last_end_time + timedelta(seconds=10) if last_end_time else datetime.now()
    
    duration = random.randint(1, 10)
    duration_time = timedelta(seconds=duration)
    end_time = start_time + duration_time
    
    result = random.choice([True, False])
    
    return (
        TrialData(
            count=count,
            trial_start_time=start_time.isoformat(),
            trial_end_time=end_time.isoformat(),
            result=result,
        ),
        end_time,
        duration,
    )

async def main():
    count = 1
    last_end_time: datetime | None = None
    
    async with httpx.AsyncClient(
        http2=True,
        timeout=httpx.Timeout(10.0),
        limits=httpx.Limits(max_connections=100)
    ) as client:
        while True:
            trial_data, last_end_time, duration = await gen_data(count, last_end_time)
            
            print(f"Trial #{trial_data.count}:")
            print(f"  Start: {trial_data.trial_start_time}")
            print(f"  End:   {trial_data.trial_end_time}")
            print(f"  Duration: {duration} s")
            print(f"  Result: {'Success' if trial_data.result else 'Failure'}")
            print("-" * 50)
            
            await send_data(client, trial_data)
            
            count += 1
            await asyncio.sleep(duration)

if __name__ == "__main__":
    asyncio.run(main())